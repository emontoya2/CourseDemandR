server <- function(input, output, session) {
  ...
  pairwiseResultsAll <- reactive({
    ...
    LD <- dataSource() %>%
      filter(GE_course_level == "LD") %>%
      pull(Req_1) %>%
      unique() ##
    UD <- dataSource() %>%
      filter(GE_course_level == "UD") %>%
      pull(Req_1) %>%
      unique() ##
  ...
  output$fillRateTimePlot <- renderPlot({
    ...
    df_plot <- dataSource() %>%
      ...
      #  Need a numeric “year” and a season order (Spring = 1, Fall = 2)
      mutate(
  ...
  output$fillRateReqPlot <- renderPlot({
    ...
    df_plot <- dataSource() %>%
      ...
      # Extract Year & season order for true chronology
      mutate(
        Year        = as.integer(paste0("20", substr(Term, 2, 3))),
        SeasonOrder = ifelse(substr(Term, 1, 1) == "S", 1, 2)
      )

    # Create factor levels: F22, S23, F23, S24, etc.
    term_levels <- df_plot %>%
      distinct(Term, Year, SeasonOrder) %>%
      arrange(Year, SeasonOrder) %>%
      pull(Term)
